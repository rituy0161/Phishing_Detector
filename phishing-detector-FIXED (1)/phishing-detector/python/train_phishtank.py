...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
Examples:
  # Basic usage (downloads legitimate URLs automatically):
  python train_phishtank.py --phishtank online-valid.csv

  # Skip re-downloading legit URLs if you already have processed data:
  python train_phishtank.py --phishtank online-valid.csv --no-download

  # Custom output folder and more training epochs:
  python train_phishtank.py --phishtank online-valid.csv --output ./my_model --epochs 100
        """
    )
    parser.add_argument("--phishtank",   required=True, help="Path to PhishTank CSV file (online-valid.csv)")
    parser.add_argument("--output",      default="./tfjs_model_phishtank", help="Output directory for TF.js model")
